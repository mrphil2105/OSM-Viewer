...
public class ZoomHandler {
    ...
    public String getZoomString() {
        if (first) {
            if (Point.geoToMap(bounds.getBottomRight()).x() - Point.geoToMap(bounds.getTopLeft()).x()
                    < Point.geoToMap(bounds.getTopLeft()).y() - Point.geoToMap(bounds.getBottomRight()).y()) {
    ...
    private float getScaleBarDistance() {
        float lon1 = bounds.left();
        float lat1 = bounds.bottom();
        float lon2 = bounds.right();
        float lat2 = bounds.bottom();
        // Haversine method
