...
public class Polygons implements Serializable {
    ...
    public void addPolygon(List<Vector2D> points, Color color) {
        // TODO: Some points might already have been added as part of another polygon.  These should not be duplicated.
        ...
        Earcut.earcut(verts).stream().map(i -> (vertices.size() - verts.length) / 2 + i).forEach(indices::add);
    }

    public void addLines(List<Vector2D> points, float width, Color color) {
        if (points.size() < 2) {
            return;
    ...
    public void addLine(Vector2D from, Vector2D to, float width, Color color) {
        var vec = from.sub(to);
