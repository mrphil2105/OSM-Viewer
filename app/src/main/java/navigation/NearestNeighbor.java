...
public class NearestNeighbor implements OSMObserver {
    ...
    public void onBounds(OSMBounds bounds) {
        twoDTree = new TwoDTree<>((float)bounds.minlon(), (float)bounds.minlat(), (float)bounds.maxlon(), (float)bounds.maxlat());
    }
    ...
    private void addToTree(Pair<SlimOSMNode, String>[] nodes, int level) {
        ...
        var point = new Point((float)median.getKey().lat(), (float)median.getKey().lon());

        if (nodes.length <= 1000) {
            var points = Arrays.stream(nodes)
                .map(p -> new Point((float)p.getKey().lat(), (float)p.getKey().lon()))
                .toList();
